...
console.log("metadata",metadataItem)
...
//problem
      if (metadataItem !== undefined) {
        const response =
         await fetch(metadataItem.data.uri);
        console.log("kkkkk",response)
        if (!response.ok) throw new Error(response.statusText);
        const json = await response.json();
        console.log("tttttt",json)
        setUriData(json);
      } else {
        setInvalidItem(true);
      }
    ...
    return (
      <Card title="Create Sale" style={{ marginBottom: 20 }}>
        <Form form={createSaleForm} layout="inline" onFinish={onCreateSale}>
          ...
          >
            <InputNumber
              addonBefore={<img src={logo} width={14} height={14} />}
              placeholder="0.0"
            />
          </Form.Item>
